        ...
        for c in self.courses:
            if course_ids:
                if c.id in course_ids:
                    course_stream_list.append(
                        get_course_stream(c.id, base_url, access_token))
            else:
                course_stream_list.append(
                    get_course_stream(c.id, base_url, access_token))
            ...
            for item in course_stream:
                if item["type"] in ["Conversation"]:
                    ...
                    if ctime_iso is None:
                        ...
                        ctime_iso_parsed = (
                            dateutil.parser.isoparse(ctime_iso)+time_shift)
                        ctime_timedelta = ctime_iso_parsed - \
                            (datetime.utcnow().replace(tzinfo=pytz.utc)+time_shift)
                        if since is not None:
                            if ctime_timedelta < -till_timedelta:
                                # since announcements are in order
                                break
                        ctime_text = ctime_iso_parsed.strftime(
                            "%Y-%m-%d %H:%M:%S")

                    data_list.append(
                        [course_name, course_url, title, url, short_desc, ctime_text, course.id])
        return data_list
        ...
        for course_assignments in courses_assignments:
            ...
            for assignment in course_assignments[1]:
                ...
                if ctime_iso is None:
                    ...
                    ctime_text = (dateutil.parser.isoparse(
                        ctime_iso)+time_shift).strftime("%Y-%m-%d %H:%M:%S")
                if dtime_iso is None:
                    dtime_text = "No info"
                else:
                    dtime_iso_parsed = (
                        dateutil.parser.isoparse(dtime_iso)+time_shift)
                    dtime_timedelta = dtime_iso_parsed - \
                        (datetime.utcnow().replace(tzinfo=pytz.utc)+time_shift)
                ...
                data_list.append([course_name, course_url, title, url,
                                  short_desc, ctime_text, dtime_text, course.id, ass_id])
